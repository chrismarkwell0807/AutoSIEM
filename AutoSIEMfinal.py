...
class ApplicationWindow(gui.mainWindow):
    ...
    def createDatabase(self, log):
        ...
        for line in logList:
            ...
            importance = 0
            badports = ["20", "21", "22", "23", "80", "443", "8080", "137", "138", "139" , "161", "162"]
            badwords = ["Attack", "ATTACK", "Leak", "LEAK", "EXPLOIT", "Exploit", "Violation", "VIOLATION", "Scan", "SCAN", "Vulnerable", "vulnerable", "VULNERABLE", ]
            goodwords = ["Potential", "POTENTIAL", "MISC", "misc", "Misc", "PING", "ping", "Ping"]
            classificationwordlist = classification.split()
            categorywordlist = category.split()
            if srcPort.startswith() in badports or srcPort.endswith() in badports:
                importance += 1
            elif dstPort.startswith() in badports or dstPort.endswith() in badports:
                importance += 1
            elif int(priority) == 2:
                importance += 1
            elif int(priority) == 1:
